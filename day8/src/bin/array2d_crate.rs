...
fn main() {
    ...
    let map = Array2D::from_rows(&str_to_array2d(&input)).unwrap();
    // let mut antenas = vec![];
    ...
    let size = (map.row_len(), map.column_len())
    // let mut antinodes: (Vec<Vec<bool>>, Vec<Vec<bool>>) = (vec![ vec![false; size.1]; size.0], vec![ vec![false; size.1]; size.0]);
    let mut antinodes = (Array2D::filled_with(false, size.0, size.1), Array2D::filled_with(false, size.0, size.1));
        ...
        for (i,(_, p1)) in same_antenas.iter().enumerate(){
            for (_, p2) in same_antenas.iter().skip(i){
                ...
                if p1 != p2 {
                    ...
                    if let Some(x) = antinodes.0.get_mut(a1.0, a1.1) { // indexing by usize onlu again! -the bound checking horror
                        *x = true;
                    }
                    if let Some(x) = antinodes.1.get_mut(a2.0, a2.1) {
                        *x = true;
                    }
